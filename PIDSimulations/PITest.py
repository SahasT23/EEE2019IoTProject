...
class MotorSimulation:
    ...
    def simulate_system(self, Kp, Ki, reference):
        """Simulate a simple DC motor with PI control"""
        # Time array
        ...
        for i in range(1, len(time)):
            ...
            integral[i] = integral[i-1] + error[i-1] * self.time_step
            
            # Calculate PI terms
            p_term[i-1] = Kp * error[i-1]
            i_term[i-1] = Ki * integral[i]
            
            # Calculate control signal (PI controller)
            control_signal[i-1] = p_term[i-1] + i_term[i-1]
        ...
        p_term[-1] = Kp * error[-1]
        i_term[-1] = Ki * integral[-1]
        control_signal[-1] = p_term[-1] + i_term[-1]
        
        return time, position, velocity, control_signal, error, p_term, i_term
    
    def create_simulation(self):
        """Create interactive simulation plot"""
        # Initial simulation
        time, position, velocity, control_signal, error, p_term, i_term = self.simulate_system(
            self.Kp, self.Ki, self.reference)
        ...
        ax_Ki = plt.axes([0.25, 0.16, 0.65, 0.03])
        ...
        # Add title
        self.fig.suptitle('PI Controller Simulation', fontsize=16)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="DC Motor PI Controller Simulation")
    ...
    # Show the simulation
    print("PI Controller Simulation")
    print("----------------------")
